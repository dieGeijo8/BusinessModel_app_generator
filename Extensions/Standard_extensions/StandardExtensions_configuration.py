...
class StandardExtensions_configuration:
    ...
    def extension_form():
        ...
        st.checkbox('Checkbox_extension', value=st.session_state.default_activate_checkbox, key='checkbox_extension')

        if st.session_state.default_activate_tab_weights == True:
            st.checkbox('Tab_weights_extension', value=st.session_state.default_activate_tab_weights, key='tab_weights_extension')

        if st.session_state.default_activate_page_weights == True:
            st.checkbox('Page_weights_extension', value=st.session_state.default_activate_page_weights, key='page_weights_extension')

    @staticmethod
    def set_extension_config():
        configuration = dict(zip([Plan.name, Percentages.name, Checkbox.name],
                                 [st.session_state['plan_extension'], st.session_state['percentage_extension'],
                                  st.session_state['checkbox_extension']]))
